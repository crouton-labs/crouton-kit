...
var require_uri_all = __commonJS((exports, module) => {
  (function(global2, factory) {
    ...
    function buildExps(isIRI2) {
      var ALPHA$$ = "[A-Za-z]", CR$ = "[\\x0D]", DIGIT$$ = "[0-9]", DQUOTE$$ = "[\\x22]", HEXDIG$$2 = merge(DIGIT$$, "[A-Fa-f]"), LF$$ = "[\\x0A]", SP$$ = "[\\x20]", PCT_ENCODED$2 = subexp(
        subexp(
          "%[EFef]" + HEXDIG$$2 + "%" + HEXDIG$$2 + HEXDIG$$2 + "%" + HEXDIG$$2 + HEXDIG$$2
        ) + "|" + subexp("%[89A-Fa-f]" + HEXDIG$$2 + "%" + HEXDIG$$2 + HEXDIG$$2) + "|" + subexp("%" + HEXDIG$$2 + HEXDIG$$2)
      ), GEN_DELIMS$$ = "[\\:\\/\\?\\#\\[\\]\\@]", SUB_DELIMS$$ = "[\\!\\$\\&\\'\\(\\)\\*\\+\\,\\;\\=]", RESERVED$$ = merge(GEN_DELIMS$$, SUB_DELIMS$$), UCSCHAR$$ = isIRI2 ? "[\\xA0-\\u200D\\u2010-\\u2029\\u202F-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFEF]" : "[]", IPRIVATE$$ = isIRI2 ? "[\\uE000-\\uF8FF]" : "[]", UNRESERVED$$2 = merge(ALPHA$$, DIGIT$$, "[\\-\\.\\_\\~]", UCSCHAR$$), SCHEME$ = subexp(
      ...
      ), H16$ = subexp(HEXDIG$$2 + "{1,4}"), LS32$ = subexp(subexp(H16$ + "\\:" + H16$) + "|" + IPV4ADDRESS$), IPV6ADDRESS1$ = subexp(subexp(H16$ + "\\:") + "{6}" + LS32$), IPV6ADDRESS2$ = subexp("\\:\\:" + subexp(H16$ + "\\:") + "{5}" + LS32$), IPV6ADDRESS3$ = subexp(
      ...
      ), ZONEID$ = subexp(subexp(UNRESERVED$$2 + "|" + PCT_ENCODED$2) + "+"), IPV6ADDRZ$ = subexp(IPV6ADDRESS$ + "\\%25" + ZONEID$), IPV6ADDRZ_RELAXED$ = subexp(
      ...
      ), PORT$ = subexp(DIGIT$$ + "*"), AUTHORITY$ = subexp(
        subexp(USERINFO$ + "@") + "?" + HOST$ + subexp("\\:" + PORT$) + "?"
      ), PCHAR$ = subexp(
        PCT_ENCODED$2 + "|" + merge(UNRESERVED$$2, SUB_DELIMS$$, "[\\:\\@]")
      ), SEGMENT$ = subexp(PCHAR$ + "*"), SEGMENT_NZ$ = subexp(PCHAR$ + "+"), SEGMENT_NZ_NC$ = subexp(
        subexp(
          PCT_ENCODED$2 + "|" + merge(UNRESERVED$$2, SUB_DELIMS$$, "[\\@]")
        ) + "+"
      ), PATH_ABEMPTY$ = subexp(subexp("\\/" + SEGMENT$) + "*"), PATH_ABSOLUTE$ = subexp(
        "\\/" + subexp(SEGMENT_NZ$ + PATH_ABEMPTY$) + "?"
      ), PATH_NOSCHEME$ = subexp(SEGMENT_NZ_NC$ + PATH_ABEMPTY$), PATH_ROOTLESS$ = subexp(SEGMENT_NZ$ + PATH_ABEMPTY$), PATH_EMPTY$ = "(?!" + PCHAR$ + ")", PATH$ = subexp(
        PATH_ABEMPTY$ + "|" + PATH_ABSOLUTE$ + "|" + PATH_NOSCHEME$ + "|" + PATH_ROOTLESS$ + "|" + PATH_EMPTY$
      ), QUERY$ = subexp(
        subexp(PCHAR$ + "|" + merge("[\\/\\?]", IPRIVATE$$)) + "*"
      ), FRAGMENT$ = subexp(subexp(PCHAR$ + "|[\\/\\?]") + "*"), HIER_PART$ = subexp(
      ...
      ), URI_REFERENCE$ = subexp(URI$ + "|" + RELATIVE$), ABSOLUTE_URI$ = subexp(
        SCHEME$ + "\\:" + HIER_PART$ + subexp("\\?" + QUERY$) + "?"
      ), GENERIC_REF$ = "^(" + SCHEME$ + ")\\:" + subexp(
        subexp(
          "\\/\\/(" + subexp("(" + USERINFO$ + ")@") + "?(" + HOST$ + ")" + subexp("\\:(" + PORT$ + ")") + "?)"
    ...
    function _normalizeComponentEncoding(components, protocol) {
      ...
      if (components.path !== void 0)
        components.path = String(components.path).replace(protocol.PCT_ENCODED, decodeUnreserved2).replace(
    ...
    var PCT_ENCODED$ = subexp(
      ...
      ) + "|" + subexp("%[89A-Fa-f]" + HEXDIG$$ + "%" + HEXDIG$$ + HEXDIG$$) + "|" + subexp("%" + HEXDIG$$ + HEXDIG$$)
    );
      ...
      serialize: function serialize$$1(mailtoComponents, options) {
        ...
        if (to) {
          for (var x = 0, xl = to.length; x < xl; ++x) {
            ...
            try {
              domain = !options.iri ? punycode.toASCII(
...
var require_validate = __commonJS((exports, module) => {
  module.exports = function generate_validate(it, $keyword, $ruleType) {
    var out = "";
    var $async = it.schema.$async === true, $refKeywords = it.util.schemaHasRulesExcept(
      it.schema,
      it.RULES.all,
      "$ref"
      ...
      if ($coerceToTypes || $typeIsArray || $rulesGroup === true || $rulesGroup && !$shouldUseGroup($rulesGroup)) {
        ...
        if ($coerceToTypes) {
          ...
          if (it.opts.coerceTypes == "array") {
            out += " if (" + $dataType + " == 'object' && Array.isArray(" + $data + ") && " + $data + ".length == 1) { " + $data + " = " + $data + "[0]; " + $dataType + " = typeof " + $data + "; if (" + it.util.checkDataType(
              it.schema.type,
              $data,
        ...
        if ($breakOnError) {
          ...
          if (arr1) {
            ...
            while ($i < l1) {
              ...
              out += ") && (missing" + $lvl + " = " + it.util.toQuotedString(
          ...
          out = "";
          if (it.createErrors !== false) {
            out += " { keyword: 'dependencies' , dataPath: (dataPath || '') + " + it.errorPath + " , schemaPath: " + it.util.toQuotedString($errSchemaPath) + " , params: { property: '" + it.util.escapeQuotes($property) + "', missingProperty: '" + $missingProperty + "', depsCount: " + $deps.length + ", deps: '" + it.util.escapeQuotes(
  ...
  module.exports = function generate__limit(it, $keyword, $ruleType) {
    ...
    if ($isDataExcl) {
      var $schemaValueExcl = it.util.getData(
        ...
        $dataLvl,
        it.dataPathArr
      ), $exclusive = "exclusive" + $lvl, $exclType = "exclType" + $lvl, $exclIsNumber = "exclIsNumber" + $lvl, $opExpr = "op" + $lvl, $opStr = "' + " + $opExpr + " + '";
  ...
  initialize() {
    try {
      ...
      this.logForDebugging(
        isNative ? `Spawning Claude Code native binary: ${spawnCommand} ${spawnArgs.join(
          " "
...
var maskSrcKey = (function() {
  var uid = /[^.]+$/.exec(
    _coreJsData_default && _coreJsData_default.keys && _coreJsData_default.keys.IE_PROTO || ""
  );
...
var reIsNative = RegExp(
  "^" + funcToString2.call(hasOwnProperty2).replace(reRegExpChar, "\\$&").replace(
...
var Query = class {
  ...
  constructor(transport, isSingleUserTurn, canUseTool, hooks, abortController, sdkMcpServers = /* @__PURE__ */ new Map()) {
    ...
    for (const [name, server] of sdkMcpServers) {
      const sdkTransport = new SdkControlServerTransport(
        (message) => this.sendMcpServerMessageToCli(name, message)
      );
...
`
      ...
      await Promise.resolve(
        this.transport.write(
          JSON.stringify(controlErrorResponse) + `
`
        )
      );
    } finally {
  ...
  async processControlRequest(request, signal) {
    if (request.request.subtype === "can_use_tool") {
      ...
      if ("method" in mcpRequest.message && "id" in mcpRequest.message && mcpRequest.message.id !== null) {
        const response = await this.handleMcpControlRequest(
...
var ClientCapabilitiesSchema = exports_external.object({
  experimental: exports_external.optional(
    exports_external.object({}).passthrough()
  ),
  sampling: exports_external.optional(
    exports_external.object({}).passthrough()
  ),
  elicitation: exports_external.optional(
    exports_external.object({}).passthrough()
  ),
  roots: exports_external.optional(
...
var ServerCapabilitiesSchema = exports_external.object({
  ...
  logging: exports_external.optional(
    exports_external.object({}).passthrough()
  ),
  completions: exports_external.optional(
    exports_external.object({}).passthrough()
  ),
  prompts: exports_external.optional(
    exports_external.object({
      listChanged: exports_external.optional(exports_external.boolean())
    }).passthrough()
  ),
  resources: exports_external.optional(
    exports_external.object({
      subscribe: exports_external.optional(exports_external.boolean()),
      listChanged: exports_external.optional(exports_external.boolean())
    }).passthrough()
  ),
  tools: exports_external.optional(
    exports_external.object({
      listChanged: exports_external.optional(exports_external.boolean())
    }).passthrough()
  )
...
var Base64Schema = exports_external.string().refine(
  (val) => {
    try {
      atob(val);
      return true;
...
var ReadResourceResultSchema = ResultSchema.extend({
  contents: exports_external.array(
...
var PromptSchema = BaseMetadataSchema.extend({
  description: exports_external.optional(exports_external.string()),
  arguments: exports_external.optional(
    exports_external.array(PromptArgumentSchema)
  ),
  _meta: exports_external.optional(exports_external.object({}).passthrough())
});
...
var GetPromptRequestSchema = RequestSchema.extend({
  ...
  params: BaseRequestParamsSchema.extend({
    name: exports_external.string(),
    arguments: exports_external.optional(
      exports_external.record(exports_external.string())
    )
  })
});
...
var ToolSchema = BaseMetadataSchema.extend({
  ...
  inputSchema: exports_external.object({
    ...
    properties: exports_external.optional(
      exports_external.object({}).passthrough()
    ),
    required: exports_external.optional(
      exports_external.array(exports_external.string())
    )
  }).passthrough(),
  outputSchema: exports_external.optional(
    exports_external.object({
      type: exports_external.literal("object"),
      properties: exports_external.optional(
        exports_external.object({}).passthrough()
...
var CallToolRequestSchema = RequestSchema.extend({
  ...
  params: BaseRequestParamsSchema.extend({
    name: exports_external.string(),
    arguments: exports_external.optional(
      exports_external.record(exports_external.unknown())
    )
  })
});
...
var ModelPreferencesSchema = exports_external.object({
  ...
  costPriority: exports_external.optional(
    exports_external.number().min(0).max(1)
  ),
  speedPriority: exports_external.optional(
    exports_external.number().min(0).max(1)
  ),
  intelligencePriority: exports_external.optional(
    exports_external.number().min(0).max(1)
  )
}).passthrough();
...
var CreateMessageRequestSchema = RequestSchema.extend({
  ...
  params: BaseRequestParamsSchema.extend({
    ...
    includeContext: exports_external.optional(
      exports_external.enum(["none", "thisServer", "allServers"])
    ),
    temperature: exports_external.optional(exports_external.number()),
    maxTokens: exports_external.number().int(),
    stopSequences: exports_external.optional(
      exports_external.array(exports_external.string())
    ),
    metadata: exports_external.optional(
      exports_external.object({}).passthrough()
    ),
...
var CreateMessageResultSchema = ResultSchema.extend({
  ...
  stopReason: exports_external.optional(
    exports_external.enum(["endTurn", "stopSequence", "maxTokens"]).or(exports_external.string())
  ),
  role: exports_external.enum(["user", "assistant"]),
...
var StringSchemaSchema = exports_external.object({
  ...
  format: exports_external.optional(
    exports_external.enum(["email", "uri", "date", "date-time"])
  )
}).passthrough();
...
var EnumSchemaSchema = exports_external.object({
  ...
  enumNames: exports_external.optional(
    exports_external.array(exports_external.string())
  )
}).passthrough();
...
var ElicitRequestSchema = RequestSchema.extend({
  ...
  params: BaseRequestParamsSchema.extend({
    ...
    requestedSchema: exports_external.object({
      type: exports_external.literal("object"),
      properties: exports_external.record(
        exports_external.string(),
...
var ElicitResultSchema = ResultSchema.extend({
  ...
  content: exports_external.optional(
    exports_external.record(
      exports_external.string(),
      exports_external.unknown()
    )
  )
});
...
var CompleteRequestSchema = RequestSchema.extend({
  ...
  params: BaseRequestParamsSchema.extend({
    ...
    context: exports_external.optional(
      exports_external.object({
        arguments: exports_external.optional(
          exports_external.record(
            exports_external.string(),
...
import { basename, dirname as dirname2, join as join4, relative } from "path";
var HOOK_NAME = "claude-md-manager";
var STATE_FILE_NAME = "claude-md-manager-cache.json";
function appendLog(message) {
  const homeDir = process.env.HOME;
  if (!homeDir) return;
  const logPath = join4(homeDir, ".claude", "logs", "hooks.log");
  const timestamp = (/* @__PURE__ */ new Date()).toISOString();
  const entry = `[${timestamp}] [${HOOK_NAME}] ${message}
`;
  try {
    writeFileSync2(logPath, entry, { flag: "a" });
  } catch (error) {
  }
}
function getStateFilePath() {
  ...
  return join4(homeDir, ".claude", "state", STATE_FILE_NAME);
}
...
function saveProcessingCache(cache) {
  ...
  try {
    ...
    writeFileSync2(statePath, JSON.stringify(cache, null, 2));
  } catch (error) {
...
function getDirectoryInfo(dirPath, cwd2) {
  if (!existsSync2(dirPath)) {
    const relativeDirPath2 = relative(cwd2, dirPath);
  ...
  const files = dirContents.filter((f) => {
    const fullPath = join4(dirPath, f);
    try {
  ...
  const relativeDirPath = relative(cwd2, dirPath);
...
function loadSettings(cwd2) {
  const homeDir = process.env.HOME;
  const excludedDirectories = [];
  if (homeDir) {
    const candidatePaths = [
      join4(homeDir, ".claude", ".claude-md-manager-ignore"),
      join4(homeDir, ".claude-md-manager-ignore")
    ];
    for (const path of candidatePaths) {
      excludedDirectories.push(...loadIgnoreFile(path));
    }
  }
  const localIgnorePath = join4(cwd2, ".claude-md-manager-ignore");
...
function getClaudeMdHierarchy(fileDir, cwd2) {
  const claudeMdHierarchy = [];
  let currentDir = fileDir;
  while (currentDir.startsWith(cwd2)) {
    const potentialClaudeMd = join4(currentDir, "CLAUDE.md");
    if (existsSync2(potentialClaudeMd) && currentDir !== fileDir) {
      ...
      claudeMdHierarchy.unshift({
        path: relative(cwd2, potentialClaudeMd),
        content,
        lineCount
      });
    }
...
async function backgroundWorker() {
  ...
  const { sessionId, cwd: cwd2, parentSessionId } = JSON.parse(input);
  const trackingSessionId = parentSessionId || sessionId;
  const homeDir = process.env.HOME;
  const lockPath = homeDir ? join4(homeDir, ".claude", "state", `claude-md-${trackingSessionId}.lock`) : null;
  ...
  if (lockPath) {
    try {
      const lockDir = dirname2(lockPath);
      execSync(`mkdir -p "${lockDir}"`, { stdio: "ignore" });
      writeFileSync2(lockPath, JSON.stringify({ sessionId, parentSessionId, timestamp: (/* @__PURE__ */ new Date()).toISOString() }));
    } catch (error) {
  ...
  let changedFiles = [];
  try {
    const gitOutput = execSync("git diff --name-only HEAD", { cwd: cwd2, encoding: "utf8" });
    changedFiles = gitOutput.trim().split("\n").filter(Boolean);
    if (changedFiles.length === 0) {
      appendLog(`[START] session=${sessionId}, cwd=${cwd2} | [SKIP] No file changes detected`);
      process.exit(0);
    }
  } catch (error) {
    appendLog(`[START] session=${sessionId}, cwd=${cwd2} | [SKIP] Git check failed or not a repo`);
    process.exit(0);
  }
  ...
  for (const file of changedFiles) {
    const filePath = join4(cwd2, file);
    const fileDir = dirname2(filePath);
    const relativePath = relative(cwd2, fileDir) || ".";
    if (relativePath.includes(".claude")) continue;
    if (basename(filePath) === "CLAUDE.md") continue;
    if (relativePath.startsWith("..")) continue;
    try {
      execSync(`git check-ignore -q "${file}"`, { cwd: cwd2, stdio: "pipe" });
      skippedGitIgnored++;
      continue;
    } catch (error) {
    }
  ...
  if (directoryFiles.size === 0) {
    ...
    appendLog(`[START] session=${sessionId}, cwd=${cwd2} | [SKIP] No new file changes since last run${detailSuffix}`);
    process.exit(0);
  }
  const { excludedDirectories } = loadSettings(cwd2);
  ...
  for (const [fileDir, filesInDir] of directoryFiles.entries()) {
    ...
    const claudeMdPath = join4(fileDir, "CLAUDE.md");
    const globalClaudePath = homeDir ? join4(homeDir, ".claude", "CLAUDE.md") : null;
    ...
    const { fileCount, fileTypes, subdirs, isRoot, targetLines } = getDirectoryInfo(fileDir, cwd2);
    if (!hasClaudeMd && !isRoot && fileCount < 4) {
      appendLog(`[SKIP] ${relativePath} (only ${fileCount} files, need \u22654)`);
      markFilesAsHandled(filesInDir, filesHandled);
      continue;
    }
    const claudeMdHierarchy = getClaudeMdHierarchy(fileDir, cwd2);
    const changedFilesInDir = filesInDir.map(({ file }) => basename(file));
    const customGuidancePath = join4(fileDir, ".claude-md-manager.md");
  ...
  if (directoriesToProcess.length === 0) {
    const finalCache2 = pruneCache(processingCache, changedFileSet);
    for (const [file, signature] of filesHandled.entries()) {
      finalCache2[file] = signature;
    }
    saveProcessingCache(finalCache2);
...
`;
    ...
    const systemPrompt = isRoot ? `You are an expert at creating comprehensive CLAUDE.md files for project root directories.
...
- Create a well-structured CLAUDE.md with clear sections` : `You are an expert at creating concise, focused CLAUDE.md files for subdirectories in software projects.
...
- Do not explain your reasoning or decision`;
    const userPrompt = hasClaudeMd ? `Files were edited in: ${relativePath}
...
Be somewhat conservative - only edit if there's a clear, important reason.` : `Files were edited in: ${relativePath}
...
async function main() {
  ...
  const cwd2 = inputData.cwd || process.cwd();
  if (!sessionId) {
    process.exit(0);
  }
